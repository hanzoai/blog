---
title: "Snow Consensus: A New Paradigm"
description: "Deep dive into the Snow consensus family powering Lux Network, achieving sub-second finality through probabilistic sampling."
date: "2020-04-22"
author: "Zach Kelling"
tags: ["consensus", "protocol", "technical"]
---

Three months ago, we introduced Lux Network. Today, we publish the technical details of our consensus protocol family: **Snow**.
...
Snow protocols occupy a new design space: permissionless like Nakamoto, fast like classical, and energy-efficient.
...
### Snowflake: Adding Confidence

Snowflake extends Slush with a confidence counter. Nodes track consecutive rounds where their preference matches the sampled majority:

```
procedure snowflake(initial_preference):
    preference = initial_preference
    confidence = 0
...
### Snowball: State Accumulation

Snowball adds persistent counters for each color, providing stronger consistency guarantees:

```
procedure snowball(initial_preference):
    preference = initial_preference
    confidence = 0
    ...
    for round in 1..max_rounds:
        ...
        if majority(votes) > alpha * k:
            ...
            if counters[majority_color] > counters[preference]:
                preference = majority_color(votes)
            // confidence logic as in snowflake
        if confidence >= beta:
            return preference
    return undecided
```

### Avalanche: DAG Structure

Avalanche builds a directed acyclic graph (DAG) of transactions, using Snowball for conflict resolution. This enables:
...
| Metric | Snow | Classical | Nakamoto |
|--------|------|-----------|----------|
...
The Snow protocols achieve safety and liveness under the following assumptions:
...
The Snow consensus implementation is complete and undergoing security audits. Testnet launch is scheduled for Q3 2020.
