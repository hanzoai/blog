...
Zen Flash was distilled from Zen Max using the Zen MoDE (Mixture of Diverse Experts) pipeline. The distillation process is not naive knowledge distillation. We used speculative decoding alignment during training, which means Zen Flash's output distribution was shaped specifically for high-throughput serving patterns. The model learns to produce correct first tokens quickly, not just correct final answers.
