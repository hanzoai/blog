...
**Zen MoDE** — Mixture of Distilled Experts — is the architectural breakthrough that defined 2025. Traditional MoE models train experts from scratch. MoDE combines three techniques: expert initialization from distillation (rather than random), Drop-Upcycling (arXiv:2502.19261) to break weight correlation between experts, and our HMM merging protocol to specialize experts post-hoc without full retraining. The result: Zen4 Max achieves 13B-class quality with 30B total parameters and only 3B active per forward pass.
